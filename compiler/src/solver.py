        ...
        if layer.get('has_variable_reg', False):
            ...
            if required_rows <= row_count:
                ...
                if layer['type'] == 'lut_quant':
                    expr2 = h_i * (layer['col'] // layer['stride']) * layer['out_channel'] * layer['quant_channels']
                else: 
                    expr2 = h_i * (layer['col'] // layer['stride']) * layer['out_channel']
                ...
                kernel_size_i = layer.get('kernel_size', 3)
                stride_i = layer.get('stride', 1)
                row_term = kernel_size_i + (h_i - 1) * stride_i
                row_term = max(1, row_term)  
                input_lut_coeff = (math.log2(max(1, col_i)) + math.log2(row_term) + 3) / 6.0
                input_lut_coeff = mainput_lut_coeff = math.ceil(input_lut_coeff)  

                padding_term = sum(next_layer.get('padding',[0,0])) * required_rows * layer.get("in_channel", 1)
                reg_base = r_i + expr2
                input_lut_count = reg_base * input_lut_coeff
                lut_total_expr += input_lut_count
                reg_base += padding_term
            else:
                if layer['type'] == 'lut_quant':
                    fixed_base_reg = row_count * (layer['col'] // layer['stride']) * layer['out_channel'] * layer['quant_channels']
                else:
                    fixed_base_reg = row_count * (layer['col'] // layer['stride']) * layer['out_channel']
                
                reg_base = fixed_base_reg
            
            if i < LAYER_NUM - 1 and h_i != layer['row']//layer['stride'] and required_rows <= row_count:
                next_layer = data['layers'][i+1]
                kernel_size_next = next_layer.get('kernel_size', 3)
                stride_next = next_layer.get('stride', 1)
                
                term1 = kernel_size_next + h_next * (stride_next - 1)
                m_row = max(term1, h_i) + h_i
                
                if m_row > 0:
                    output_lut_per_bit = math.ceil(math.ceil(math.ceil(math.log2(m_row)) + m_row) / 6.0) * 2.0 / 3.0
                    output_lut_per_bit = math.ceil(output_lut_per_bit)
            ...
            if required_rows <= row_count:
                log_term = math.log2(next_layer['col']) * 2 + math.ceil(math.log2(next_layer['row']) * 2.5)
            ...
            if layer['type'] in ['lut_conv', 'lut_res', 'lut_quant']:
                ...
                if layer['col'] > 0 and layer['row'] > 0:
                    log_term = math.log2(layer['col']) * 2 + math.ceil(math.log2(layer['row']) * 2.5)
    ...
    for i, layer in enumerate(data['layers']):
        ...
        if layer['type'] in ['lut_conv', 'lut_res', 'lut_quant']:
            ...
            if i < LAYER_NUM - 1 and data['layers'][i+1]['type'] == 'lut_fc':
                ...
                h_i = h_config[i]
                bottom_padding_blocks = bottom_padding / float(h_i) if h_i > 0 else 0
    ...
    for i in range(LAYER_NUM):
        ...
        if layer.get('has_variable_reg', False) and i < LAYER_NUM - 1:
            ...
            if required_rows <= row_count:
                ...
                padding_term = sum(next_layer.get('padding',[0,0])) * required_rows * layer['in_channel']
                base_reg = max(expr1, expr2) + expr2
                
                col_i = layer['col']
                kernel_size_i = layer.get('kernel_size', 3)
                stride_i = layer.get('stride', 1)
                row_term = kernel_size_i + (h_choice - 1) * stride_i
                row_term = max(1, row_term)
                input_lut_coeff = (math.log2(max(1, col_i)) + math.log2(row_term) + 3) / 6.0
                input_lut_coeff = math.ceil(input_lut_coeff)  
                
                input_lut = base_reg * input_lut_coeff
                base_reg += padding_term
            else:
                if layer['type'] == 'lut_quant':
                    base_reg = row_count * (layer['col'] // layer['stride']) * layer['out_channel'] * layer['quant_channels']
                else:
                    base_reg = row_count * (layer['col'] // layer['stride']) * layer['out_channel']
                input_lut = 0  
            
            layer['opt_input_lut_usage'] = int(input_lut)
            total_lut_used += int(input_lut)
            
            if i < LAYER_NUM - 1 and h_choice != layer['row']//layer['stride'] and required_rows <= row_count:
                next_layer = data['layers'][i+1]
                kernel_size_next = next_layer.get('kernel_size', 3)
                stride_next = next_layer.get('stride', 1)
                w_next = w_choices[i+1]
                
                term1 = kernel_size_next + next_h * (stride_next - 1)
                m_row = max(term1, h_choice) + h_choice
                ...
                if m_row > 0:
                    ...
                    output_lut_coeff = ((kernel_size_next + (next_h-1) * stride_next )/(kernel_size_next*next_h)) 
                    output_lut = output_lut_per_bit * output_total_bits * output_lut_coeff
            ...
            if required_rows <= row_count:
                log_reg = 0
                log_reg = math.log2(next_layer['col']) * 2 + math.ceil(math.log2(next_layer['row']) * 2.5) 
    ...
    for i, layer in enumerate(base_data['layers']):
        ...
        if layer.get('has_variable_reg', False) and i < LAYER_NUM - 1:
            ...
            if required_rows <= row_count:
                ...
                input_lut = base_reg * input_lut_coeff
                padding_term = sum(next_layer.get('padding',[0,0])) * required_rows * layer['out_channel']
            ...
            if required_rows <= row_count:
                log_reg = 0
                log_reg = math.log2(next_layer['col']) * 2 + math.ceil(math.log2(layer['row']) * 2.5)
                base_reg = base_reg + log_reg
...
def heuristic_h_search_best(json_path, limit, mode='steepest', random_seed=None):
    ...
    if (solution_data and 
        ...
        print(f"!!! base H_CONFIG {h_current_best} solver failed.")
        return None
