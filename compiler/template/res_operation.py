...
def generate_res_operation(rtl_path,
    ...
    for idx_r in range(kernel_num_row):
        for idx_c in range(kernel_num_col):
            ...
            rtl_code.append(");\n")
            rtl_code.append("\n")
            generate_res_kernel(rtl_path, module_name+"_res_kernel_r"+str(idx_r)+"_c"+str(idx_c), lut_num_per_channel, in_bit_width, out_bit_width, stride_size, channel_in, channel_out, lut_size, lut_weights, threshold, packing, lut_packing, lut_pin_active)
