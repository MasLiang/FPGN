...
def generate_conv_operation(rtl_path,
    ...
    for idx_r in range(kernel_num_row):
        for idx_c in range(kernel_num_col):
            ...
            rtl_code.append(");\n")
            rtl_code.append("\n")
            generate_conv_kernel(rtl_path, module_name+"_conv_kernel_r"+str(idx_r)+"_c"+str(idx_c), lut_num_per_channel, channel_out, lut_size, lut_weights, threshold, packing, lut_packing, lut_pin_active)
