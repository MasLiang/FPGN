...
def generate_popcount(rtl_path, module_name, width):
    ...
    rtl_code.append("    input  wire ["+str(width)+"-1:0] data_i,         \n")
    rtl_code.append("    output wire ["+str(width_bits)+"-1:0] data_o  \n")
    rtl_code.append(");\n")
    rtl_code.append("\n")
    ...
    def generate_adder_tree(elements, level=0, width=64, width_bits=7):
        ...
        if level == 0:
            ...
            for i in range(elements//2):
                code.append("assign stage"+str(level+1)+"["+str(i)+"] = {"+str(width_bits-3)+"'b0, lut6_results["+str(2*i)+"]} + ")
                code.append("{"+str(width_bits-3)+"'b0, lut6_results["+str(2*i+1)+"]};\n")
            if elements % 2 == 1:
                code.append("assign stage"+str(level+1)+"["+str(elements//2)+"] = {"+str(width_bits-2)+"'b0, lut6_results["+str(elements-1)+"]};\n")
            code.append("\n")
        else:
            code.append("    wire ["+str(width_bits)+"-1:0] stage"+str(level+1)+" [0:"+str(next_level_elements-1)+"];\n")
            for i in range(elements//2):
