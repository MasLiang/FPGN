...
def generate_buffer_to_fc(rtl_path,
    ...
    rtl_code.append("end\n")
    rtl_code.append("\n")
    row_block = row // in_kernel_num_row
    col_block = col // in_kernel_num_col
    for rb in range(row_block):
        for cb in range(col_block):
            for r in range(in_kernel_num_row):
                for c in range(in_kernel_num_col):
                    rtl_code.append("always @(posedge clk)\n")
                    rtl_code.append("begin\n")
                    rtl_code.append("   if(write_row_addr=="+str(rb*in_kernel_num_row)+" && write_col_addr=="+str(cb*in_kernel_num_col)+")\n")
                    rtl_code.append("       mem_blocks["+str(rb*in_kernel_num_row+r)+"]["+str(cb*in_kernel_num_col+c)+"] <= data_i["+str((r*in_kernel_num_col+c)*channel)+"+:"+str(channel)+"];\n")
                    rtl_code.append("end\n")
