...
def generate_buffer_res_to_res(rtl_path,
    ...
    if stride_size < kernel_size:
        ...
        safety_margin = 0
    mem_row = min(max(in_kernel_num_row, (kernel_size+(out_kernel_num_row-1)*stride_size))+max(in_kernel_num_row, out_kernel_num_row*stride_size), row)
    res_row = min(max(in_kernel_num_row, (stride_size+(out_kernel_num_row-1)*stride_size))+max(in_kernel_num_row, out_kernel_num_row*stride_size), row)
    mem_col = col
    ...
    decl_code.append('(* DONT_TOUCH = "yes" *)\n')
    decl_code.append("reg ["+str(channel*in_bit_width)+"-1:0] res_blocks [0:"+str(res_row)+"-1][0:"+str(mem_col)+"-1];\n")
    decl_code.append('(* DONT_TOUCH = "yes" *)\n')
    ...
    decl_code.append("wire [("+str(mem_row_bit)+")-1:0] write_row_addr_pre;\n")
    # decl_code.append('(* DONT_TOUCH = "yes" *)\n')
    decl_code.append("(* MAX_FANOUT = 100 *)\n")
    ...
    rtl_code.append("            begin\n")
    rtl_code.append("                write_row_addr <= "+str(padding_size//2+in_kernel_num_row)+";\n")
    rtl_code.append("            end\n")
    rtl_code.append("            else if (read_row_add_en) \n")
    rtl_code.append("            begin\n")
    if in_kernel_num_row>out_kernel_num_row*stride_size:
        rtl_code.append("                write_row_addr <= write_row_addr + "+str(in_kernel_num_row-out_kernel_num_row*stride_size)+";\n")
    else:
        rtl_code.append("                write_row_addr <= write_row_addr - "+str(out_kernel_num_row*stride_size-in_kernel_num_row)+";\n")
    rtl_code.append("            end\n")
    ...
    rtl_code.append("end\n")
    rtl_code.append("\n")
    rtl_code.append("assign write_row_addr_pre = read_row_add_en ? (write_row_addr - "+str(out_kernel_num_row*stride_size)+") : write_row_addr;\n")
    row_addr_start_lst = []
    row_addr_end_lst = []
    col_addr_start_lst = []
    col_addr_end_lst = []
    for mr in range(padding_size//2, mem_row, in_kernel_num_row):
        start_addr = mr
        while start_addr>=0:
            row_addr_start_lst.append(start_addr)
            start_addr -= out_kernel_num_row*stride_size
    ...
    for mr in range(mem_row):
        for mc in range(mem_col):
            ...
            for wrt_en_idx in range(len(wrt_en_addr_lst)):
                ...
                assign_code.append("assign mem_blocks_wen_"+str(mr)+"_"+str(mc)+"_"+str(row_bias)+"_"+str(col_bias)+" = write_row_addr_pre=="+str(row_start_addr)+" && write_col_addr=="+str(col_start_addr)+";\n")
                if wrt_en_idx==0:
                    if mr<padding_size//2:
                ...
                rtl_code.append("    end\n")
            if mr<(out_kernel_num_row-1)*stride_size+kernel_size:
                rtl_code.append("    else if (data_o_vld&read_row_add_en) \n")
                rtl_code.append("    begin\n")
                final_shift = shift_per_time*(shift_times-1)
                new_col = mc - final_shift
                new_row = mr + out_kernel_num_row*stride_size
                if new_row<(out_kernel_num_row-1)*stride_size+kernel_size:
                    if new_col==0:
                        rtl_code.append("        mem_blocks["+str(mr)+"]["+str(mc)+"] <= mem_blocks_padding["+str(new_row)+"][0];\n")
                    elif new_col==-1:
                        rtl_code.append("        mem_blocks["+str(mr)+"]["+str(mc)+"] <= mem_blocks_padding["+str(new_row)+"][1];\n")
                    elif new_col< 0:
                        new_col = new_col + col+1
                        rtl_code.append("        mem_blocks["+str(mr)+"]["+str(mc)+"] <= mem_blocks["+str(new_row)+"]["+str(new_col)+"];\n")
                    else:
                        rtl_code.append("        mem_blocks["+str(mr)+"]["+str(mc)+"] <= mem_blocks["+str(new_row)+"]["+str(new_col)+"];\n")
                else:
                    rtl_code.append("        mem_blocks["+str(mr)+"]["+str(mc)+"] <= mem_blocks["+str(new_row)+"]["+str(mc)+"];\n")
                rtl_code.append("    end\n")
            ...
            rtl_code.append("end\n")
            rtl_code.append("\n")
    for mr in range(res_row):
        for mc in range(mem_col):
            wrt_en_addr_lst = []
            for row_addr_idx in range(len(row_addr_start_lst)):
                row_start_addr = row_addr_start_lst[row_addr_idx]-padding_size//2
                row_end_addr = row_addr_end_lst[row_addr_idx]-padding_size//2
                if row_start_addr<=row_end_addr:
                    if mr>=row_start_addr and mr<=row_end_addr:
                        for col_addr_idx in range(len(col_addr_start_lst)):
                            col_start_addr = col_addr_start_lst[col_addr_idx]
            ...
            for wrt_en_idx in range(len(wrt_en_addr_lst)):
                row_start_addr, col_start_addr, row_bias, col_bias = wrt_en_addr_lst[wrt_en_idx]
                decl_code.append("wire res_blocks_wen_"+str(mr)+"_"+str(mc)+"_"+str(row_bias)+"_"+str(col_bias)+";\n")
                assign_code.append("assign res_blocks_wen_"+str(mr)+"_"+str(mc)+"_"+str(row_bias)+"_"+str(col_bias)+" = write_row_addr_pre=="+str(row_start_addr+padding_size//2)+" && write_col_addr=="+str(col_start_addr)+";\n")
                if wrt_en_idx==0:
                    rtl_code.append("    if (data_i_vld && res_blocks_wen_"+str(mr)+"_"+str(mc)+"_"+str(row_bias)+"_"+str(col_bias)+") \n")
                else:
                    rtl_code.append("    else if (data_i_vld && res_blocks_wen_"+str(mr)+"_"+str(mc)+"_"+str(row_bias)+"_"+str(col_bias)+") \n")
                rtl_code.append("    begin\n")
                rtl_code.append("        res_blocks["+str(mr)+"]["+str(mc)+"] <= res_i["+str((row_bias*in_kernel_num_col+col_bias+1)*channel*in_bit_width)+"-1:"+str((row_bias*in_kernel_num_col+col_bias)*channel*in_bit_width)+"]; \n")
                rtl_code.append("    end\n")
            if mr<(out_kernel_num_row-1)*stride_size+stride_size:
                rtl_code.append("    else if (data_o_vld&read_row_add_en) \n")
                rtl_code.append("    begin\n")
                final_shift = shift_per_time*(shift_times-1)
                new_row = mr + out_kernel_num_row*stride_size
                if new_row<(out_kernel_num_row-1)*stride_size+stride_size:
                    new_col = mc - final_shift
                    if new_col< 0:
                        new_col = new_col + col
                    rtl_code.append("        res_blocks["+str(mr)+"]["+str(mc)+"] <= res_blocks["+str(new_row)+"]["+str(new_col)+"];\n")
                else:
                    rtl_code.append("        res_blocks["+str(mr)+"]["+str(mc)+"] <= res_blocks["+str(new_row)+"]["+str(mc)+"];\n")
                rtl_code.append("    end\n")
                rtl_code.append("    else if(data_o_vld)\n")
                rtl_code.append("    begin\n")
                new_col = mc+shift_per_time
                if new_col>= col:
                    new_col = new_col - col
                rtl_code.append("        res_blocks["+str(mr)+"]["+str(mc)+"] <= res_blocks["+str(mr)+"]["+str(new_col)+"];\n")
                rtl_code.append("    end\n")
            else:
                new_row = mr + out_kernel_num_row*stride_size
                if new_row<res_row:
            ...
            for channel_idx in range(channel):
                for k_c in range(kernel_size):
                    for k_r in range(kernel_size):
                        if rd_c*stride_size+k_c<padding_size//2:
                            rtl_code.append("assign data_o["+str((((rd_r*out_kernel_num_col+rd_c)*channel+channel_idx)*kernel_size+k_c)*kernel_size+k_r)+"] = mem_blocks_padding["+str(rd_r*stride_size+k_r)+"][0]["+str(channel_idx)+"];\n")
                        elif rd_c*stride_size+k_c>=col+padding_size//2:
                            rtl_code.append("assign data_o["+str((((rd_r*out_kernel_num_col+rd_c)*channel+channel_idx)*kernel_size+k_c)*kernel_size+k_r)+"] = mem_blocks_padding["+str(rd_r*stride_size+k_r)+"][1]["+str(channel_idx)+"];\n")
                        else:
                            rtl_code.append("assign data_o["+str((((rd_r*out_kernel_num_col+rd_c)*channel+channel_idx)*kernel_size+k_c)*kernel_size+k_r)+"] = mem_blocks["+str(rd_r*stride_size+k_r)+"]["+str(rd_c*stride_size+k_c)+"]["+str(channel_idx)+"];\n")
    for rd_r in range(out_kernel_num_row):
        for rd_c in range(out_kernel_num_col):
            for channel_idx in range(channel):
                 for s_c in range(stride_size):
                    for s_r in range(stride_size):
                        rtl_code.append("assign res_o["+str(in_bit_width*((((rd_r*out_kernel_num_col+rd_c)*channel+channel_idx)*stride_size+s_c)*stride_size+s_r)+in_bit_width-1)+":"+str(in_bit_width*((((rd_r*out_kernel_num_col+rd_c)*channel+channel_idx)*stride_size+s_c)*stride_size+s_r))+"] = res_blocks["+str(rd_r*stride_size+s_r)+"]["+str(rd_c*stride_size+s_c)+"]["+str(channel_idx)+"];\n")
