...
def interp6_alpha_einsum(x, w, reverse_bits=False):
    assert x.ndim == 5 and x.size(-1) == 6, f"x must be [B,R,O,L,6], got {tuple(x.shape)}"
    x = x.contiguous()
    dtype = x.dtype
    B, R, O, L, _ = x.shape
    w = w.to(dtype)
    if w.ndim == 5:
        assert w.shape == (1, 1, O, L, 64)
        w = w[0, 0]
    elif w.ndim == 3:
        assert w.shape == (O, L, 64)
    else:
        assert w.numel() == O * L * 64
        w = w.reshape(O, L, 64)
    w = w.contiguous()

    BR = B * R
    xf = x.view(BR, O, L, 6).contiguous()
    g = torch.stack([1 - xf, xf], dim=-1)

    res = w.view(1, O, L, 2, 2, 2, 2, 2, 2)

    for k in range(6):
        gk = g[..., 5 - k, :]
        need = res.ndim - gk.ndim
        for _ in range(need):
            gk = gk.unsqueeze(-2)
        gk = gk.contiguous()
        #new_shape = list(gk.shape[:-1])+[1]*(res.ndim-gk.ndim-1)+[2]
        #gk = gk.reshape(new_shape).contiguous()
        res = (res * gk).sum(dim=-1)
    y = res.view(B, R, O, L)                       # [B,R,O,L]
    return y

lut_forward = torch.compile(interp6_alpha_einsum, mode='max-autotune-no-cudagraphs', dynamic=True)
#lut_forward = torch.compile(lut_cell_forward, mode='max-autotune-no-cudagraphs', dynamic=True)
...
class lut_kernel(nn.Module):
    def __init__(self, in_num, out_num, lut_k=6, use_checkpoint=True):
        ...
        self.register_buffer('mask', torch.ones(out_num, self.lut_num, lut_k))

    def get_input_sensitivity(self, hard=False, l1l2='l1'):
        O, L, N = self.w.shape
        K = self.lut_k
        sensitivities = []
        ...
        for k in range(K):
            ...
            indices = torch.arange(N, device=self.w.device)
            mask_0 = (indices & stride) == 0
            mask_1 = (indices & stride) != 0
            
            w_0 = binary_gumbel_softmax(self.w[..., mask_0], tau=self.tau, hard=hard)
            w_1 = binary_gumbel_softmax(self.w[..., mask_1], tau=self.tau, hard=hard)
            
            if l1l2=='l2':
                diff = (w_0 - w_1).pow(2).mean(dim=-1)
            elif l1l2=='l1':
                diff = torch.abs(w_0 - w_1).mean(dim=-1)
            sensitivities.append(diff)
            
        return torch.stack(sensitivities, dim=-1)

    @torch.no_grad()
    def prune_input_physical(self):
        O, L, N = self.w.shape
        K = self.lut_k
        ...
        for k in range(K):
            ...
            stride = 1 << k
            indices = torch.arange(N, device=self.w.device)
            idx_0 = (indices & stride) == 0
            idx_1 = (indices & stride) != 0
            ...
            mask_k_exp = mask_k.unsqueeze(-1).expand_as(w_avg)
            
            w_0_new = torch.where(mask_k_exp, w_avg, w_0)
            w_1_new = torch.where(mask_k_exp, w_avg, w_1)
            
            w_full[..., idx_0] = w_0_new
            w_full[..., idx_1] = w_1_new
    ...
    def forward(self, x):
        ...
        y = self._checkpoint_forward(x, self.w)
    ...
    def _checkpoint_forward(self, x, w):
        ...
        x = x * self.mask.unsqueeze(0).unsqueeze(1)
        w_q = binary_gumbel_softmax(w, tau=self.tau, hard=self.hard)
        #y = lut_cell_forward(x, w_q)
        #y = self.lut_jit(x, w_q)
        
        #y = interp6_alpha_einsum(x, w_q)
...
class lut_fc(nn.Module):
    ...
    def forward(self,x):
        ...
        """
        #aaa = torch.compile(self.compile_try, dynamic=True)
        #return aaa(x)
...
class lut_conv_func(nn.Module):
    def __init__(self, in_num, out_channel, lut_k=6, bias=True):
        ...
        self.lut_k = lut_k
        #self.total_in_num = self.lut_k*in_num 
